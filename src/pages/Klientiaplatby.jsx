...
import { listClients, updateClient, listPayments } from '../api/queries'
...
function Klientiaplatby() {
  ...
  const [emailPopover, setEmailPopover] = useState({ open: false, jmeno: null, value: '' })
  const [phonePopover, setPhonePopover] = useState({ open: false, jmeno: null, value: '' })
  ...
  const filteredPlatby = useMemo(() => {
    ...
    return list
  }, [platbyData, platbyPeriod, periodFrom, periodTo, platbyChatter, platbyPlatform])
  ...
  const handleEmailClick = (e, row) => {
    ...
    setEmailPopover({ open: true, jmeno: row.jmeno, value: current })
    setPhonePopover({ open: false, jmeno: null, value: '' })
  }

  const handlePhoneClick = (e, row) => {
    e.stopPropagation()
    const current = phoneMap[row.jmeno] ?? row.phone ?? ''
    setPhonePopover({ open: true, jmeno: row.jmeno, value: current })
    setEmailPopover({ open: false, jmeno: null, value: '' })
  }

  const saveEmail = async () => {
    if (!emailPopover.jmeno) return
    const row = klientiData.find(r => r.jmeno === emailPopover.jmeno)
    try {
      if (row?.id) await updateClient(TEAM_ID, row.id, { email: emailPopover.value })
      setEmailMap((prev) => ({ ...prev, [emailPopover.jmeno]: emailPopover.value }))
    } catch (e) {
      // eslint-disable-next-line no-alert
      alert('Uložení emailu selhalo')
    }
    setEmailPopover({ open: false, jmeno: null, value: '' })
  }

  const savePhone = async () => {
    if (!phonePopover.jmeno) return
    const row = klientiData.find(r => r.jmeno === phonePopover.jmeno)
    try {
      if (row?.id) await updateClient(TEAM_ID, row.id, { phone: phonePopover.value })
      setPhoneMap((prev) => ({ ...prev, [phonePopover.jmeno]: phonePopover.value }))
    } catch (e) {
      // eslint-disable-next-line no-alert
      alert('Uložení telefonu selhalo')
    }
    setPhonePopover({ open: false, jmeno: null, value: '' })
  }
  ...
  const saveNotes = async () => {
    ...
    try {
      ...
      // eslint-disable-next-line no-alert
      alert('Uložení poznámek selhalo')
    }
    closeNotes()
  }
  ...
  const filteredAndSortedClients = useMemo(() => {
    ...
    list = list.filter((r) => {
      const has = Number.isFinite(r.vyplata) && r.vyplata > 0
    ...
    if (payoutDayFilter !== 'all') {
      ...
      list = list.filter((r) => Number(r.vyplata) === day)
    }
    ...
    return list
  }, [klientiData, searchQuery, lastSentFilter, salaryFilter, sortBy])

  return (
    <div className="h-screen overflow-y-auto p-4 md:p-6 ml-64">
        ...
        {activeTab === 'klienti' && (
          <div className="mb-3 grid grid-cols-1 md:grid-cols-5 gap-2">
            <input
              ...
              className="col-span-1 md:col-span-2 bg-obsidian border border-velvet-gray rounded-lg px-3 py-2 text-pearl placeholder-pearl/50 focus:border-neon-orchid focus:shadow-glow-purple outline-none"
              value={searchQuery}
            ...
            <select
              className="bg-obsidian border border-velvet-gray rounded-lg px-3 py-2 text-pearl focus:border-neon-orchid focus:shadow-glow-purple outline-none w-32"
        ...
        {activeTab === 'klienti' ? (
            ...
            <table className="w-full text-left text-sm text-pearl">
              <thead className="text-pearl/80">
                <tr className="border-b border-velvet-gray">
                  ...
                  <th className="p-3">Kolik poslal za posledních 30 dnu</th>
                  <th className="p-3">Kolik poslal celkově</th>
                </tr>
              </thead>
              <tbody>
                {filteredAndSortedClients.map((row) => (
                  ...
                  <tr
                    ...
                    onClick={() => setExpandedClient((prev) => prev === row.jmeno ? null : row.jmeno)}
                  >
                    ...
                    <td className="p-3 text-center relative">
                      ...
                      {emailPopover.open && emailPopover.jmeno === row.jmeno && (
                        <div className="absolute top-10 left-1/2 -translate-x-1/2 z-50 bg-gray-800 border border-pink-400/40 rounded-lg p-3 shadow-md w-64">
                          <div className="text-pearl text-sm mb-2">Upravit email</div>
                          <input
                            type="email"
                            className="w-full bg-obsidian border border-velvet-gray rounded px-2 py-1 text-pearl mb-2 focus:border-neon-orchid focus:shadow-glow-purple outline-none"
                            value={emailPopover.value}
                            onChange={(e) => setEmailPopover((prev) => ({ ...prev, value: e.target.value }))}
                          />
                          <div className="flex justify-end gap-2">
                            <button className="px-3 py-1 rounded bg-velvet-gray text-pearl" onClick={(e) => { e.stopPropagation(); setEmailPopover({ open: false, jmeno: null, value: '' }) }}>Zavřít</button>
                            <button className="px-3 py-1 rounded bg-gradient-to-r from-neon-orchid to-crimson text-white shadow-glow-purple" onClick={(e) => { e.stopPropagation(); saveEmail() }}>Uložit</button>
                          </div>
                        </div>
                      )}
                    </td>
                    <td className="p-3 text-center relative">
                      ...
                      {phonePopover.open && phonePopover.jmeno === row.jmeno && (
                        <div className="absolute top-10 left-1/2 -translate-x-1/2 z-50 bg-gray-800 border border-pink-400/40 rounded-lg p-3 shadow-md w-64">
                          <div className="text-pearl text-sm mb-2">Upravit telefon</div>
                          <input
                            type="tel"
                            className="w-full bg-obsidian border border-velvet-gray rounded px-2 py-1 text-pearl mb-2 focus:border-neon-orchid focus:shadow-glow-purple outline-none"
                            value={phonePopover.value}
                            onChange={(e) => setPhonePopover((prev) => ({ ...prev, value: e.target.value }))}
                          />
                          <div className="flex justify-end gap-2">
                            <button className="px-3 py-1 rounded bg-velvet-gray text-pearl" onClick={(e) => { e.stopPropagation(); setPhonePopover({ open: false, jmeno: null, value: '' }) }}>Zavřít</button>
                            <button className="px-3 py-1 rounded bg-gradient-to-r from-neon-orchid to-crimson text-white shadow-glow-purple" onClick={(e) => { e.stopPropagation(); savePhone() }}>Uložit</button>
                          </div>
                        </div>
                      )}
                    </td>
                              ...
                              <tbody>
                                {(row.transakce || []).map((t, i) => (
                                  <tr key={i} className="border-b border-velvet-gray/40">
                                    ...
                                    <td className="p-2">{t.co}</td>
                                    <td className="p-2">{t.platforma}</td>
                                    <td className="p-2 whitespace-nowrap">{t.datum}</td>
                                  </tr>
                                ))}
                              </tbody>
                            </table>
          ...
          <>
            ...
            <div className="mb-3 flex flex-wrap items-end gap-3">
              ...
              <div className="ml-auto text-2xl font-bold text-sunset-gold" style={{ textShadow: '0 0 8px rgba(255, 215, 0, 0.6)' }}>
                Celková částka: {new Intl.NumberFormat('cs-CZ', { style: 'currency', currency: 'CZK', maximumFractionDigits: 0 }).format(totalPlatbyCZK)}
              </div>
            </div>
